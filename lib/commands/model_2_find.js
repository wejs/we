module.exports = function findModelsCommand(program) {
  /**
  ...
  program
  .command('model:find <modelName> [id]')
  .option('-w, --where <jsonObject>', 'Search with where')
  .description('Search for one model record')
  .action(function run(modelName, id, opts) {
    ...
    we.bootstrap(function (err) {
      ...
      if (!we.db.models[modelName]) {
        return doneAll('model with name '+modelName+' not found')
      }
